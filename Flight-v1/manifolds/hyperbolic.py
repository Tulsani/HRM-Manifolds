...
class MobiusLinear(nn.Module):
    def __init__(self, in_features: int, out_features: int, curvature: float) -> None:
        super().__init__()
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        projected = F.linear(tangent, self.weight, self.bias)
...
class HyperbolicExpert(ManifoldExpert):
    ...
    def distance(self, z1: Tensor, z2: Tensor) -> Tensor:
        return poincare_distance(z1.unsqueeze(1), z2.unsqueeze(0), self.curvature)
    ...
    def prototype_loss(self, h: Tensor, target_prototype_ids: Tensor, margin: float = 2.0) -> Tensor:
        z = self.encode(h)
        return self.prototype_memory.prototype_loss(z, target_prototype_ids, self.curvature, margin=margin)

    def expert_config(self) -> dict[str, float | int | str]:
        config = super().expert_config()
        config.update({"h_dim": self.h_dim, "n_prototypes": self.prototype_memory.n_prototypes})
        return config
